...
class S3Worker(Singleton):
    ...
    def get_files_urls(self, bucket_name: str, files_names: list[str]) -> dict[str, str]:
        """
        :param bucket_name: name of the bucket, where the files will be uploaded
        :param files_names: names of the files
        """
        return {
            file_name: self.get_file_url(bucket_name, file_name)
